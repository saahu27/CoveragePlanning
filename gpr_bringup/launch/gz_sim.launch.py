"""Gazebo sim + TurtleBot spawn for GPR coverage worlds."""
...
def generate_launch_description():
    ...
    declare_world_cmd = DeclareLaunchArgument(
        "world",
        default_value=os.path.join(bringup_dir, "worlds", "gpr_scan.sdf.xacro"),
    ...
    if turtlebot_model_os_value == "3":
        gz_bridge_config = os.path.join(bringup_dir, "configs", "turtlebot3_bridge.yaml")
        urdf = os.path.join(
            get_package_share_directory("nav2_minimal_tb3_sim"),
            "urdf",
            "turtlebot3_waffle.urdf",
        )
        with open(urdf, "r", encoding="utf-8") as info:
            robot_description = info.read()
    else:
        gz_bridge_config = os.path.join(bringup_dir, "configs", "turtlebot4_bridge.yaml")
    ...
    world_sdf = tempfile.mktemp(prefix="gpr_", suffix=".sdf")
    world_sdf_xacro = ExecuteProcess(cmd=["xacro", "-o", world_sdf, world])
    gazebo = ExecuteProcess(cmd=["gz", "sim", "-r", world_sdf], output="screen")
